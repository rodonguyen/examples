...
top -d 0.1 | grep "mnist_cnn" > mem_usage.log
top -d 0.1 -o +%MEM | grep "mnist_cnn" > mem_usage.log
# ./${output} > console_output_01_convo.txt | top -b -d 0.1 -o +%MEM | grep " mnist_cnn" > mem_usage.log
# top -b -d 1 -o +%MEM | grep -A 9 "top - " > mem_usage.log
# top -b -d 5 -o +%MEM | grep -A 4 "top - " > mem_usage.log

# top -bd 0.5 -o +%CPU | grep "top - \|python3.9" > cpu_usage.log
# top -bd 0.5 -o +%CPU | grep "load average" -A 9 > memory_cpu_usage.loglog
